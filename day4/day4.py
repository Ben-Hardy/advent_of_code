...
class Bingo:
	def __init__(self, board):
		self.bingo_board = np.array(board)
	...
	def check_win(self):
		...
		#if np.sum(np.diagonal(self.mask)) == 5 or np.sum(np.fliplr(self.mask).diagonal()) == 5:
		#	return True
	...
	def calculate_score(self, winning_call):
		cumulative_total = 0
		...
		return cumulative_total * winning_call
...
class TestBingo(unittest.TestCase):
	...
	'''
		...
		# test right-to-left diagonal win detection
		for i in range(5):
			b.mask[i,i] = 0
			b.mask[i, 4 - i] = 1
		assert_equal(True, b.check_win())
	...
	def test_find_loc(self):
		...
		assert_equal(None, b.find_loc(26)) 
